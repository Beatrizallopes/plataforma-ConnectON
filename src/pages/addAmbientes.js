...
const addAmbientes= ({route,navigation}) => {
  ...
  const itemLista = grupos.map((grupo) => {
      ...
      const item = grupo.map((ambiente) => {
        ...
        for(var m=0;m<automação.ambientes.length;m++){
          if(automação.ambientes[m] == ambiente.nome){
              jaSel = true;
              z = m;
          }   
        }
        ...
        if(selecionado){
          ...
          if(!jaSel){
            ...
            automação.ambientes.push(grupos[i][j].nome);
          }        
        } else {
          var urlSelecionado = require('./../images/icons/ambNaoSel.png');
          if(jaSel){
            automação.ambientes.splice(z,1);
          }
        }
       ...
       <Modal animationType="slide" transparent={true} visible={true} >
        <View style={styles.centeredView}>
          <View style={styles.modalSelecionarAmb}>
            <TouchableWithoutFeedback onPress={() => navigation.navigate("Automação",{automação:automação})}>
