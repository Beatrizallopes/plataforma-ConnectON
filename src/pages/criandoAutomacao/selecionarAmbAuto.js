...
const selecionarAmbientesAuto = ({route,navigation}) => {
  ...
  const itemLista = grupos.map((grupo) => {
      ...
      const item = grupo.map((ambiente) => {
        ...
        for(var m=0;m<automação.ambientesSel.length;m++){
          if(automação.ambientesSel[m].grupo == i && automação.ambientesSel[m].ambiente == j){
              jaSel = true;
              z = m;
          }   
        }
        ...
        if(selecionado){
          ...
          if(!jaSel){
            ...
            automação.ambientesSel.push(elemento);
          }        
        } else {
          var urlSelecionado = require('./../../images/icons/ambNaoSel.png');
          if(jaSel){
            automação.ambientesSel.splice(z,1);
          }
        }
...
var indicador = function(myColor, tipo) {
  ...
  export default selecionarAmbientesAuto;